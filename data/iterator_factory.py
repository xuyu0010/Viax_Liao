...
def get_ucf101_dark(data_root='./dataset/UCF101_dark',
			   clip_length=8,
			   train_interval=2,
			   val_interval=2,
			   mean=[0.485, 0.456, 0.406],
...
def get_aid11(data_root='./dataset/AID11',
	...
	train_sampler = sampler.RandomSampling(num=clip_length,
										   ...
										   seed=(seed+0))
	train = VideoIter(video_prefix=os.path.join(data_root, 'raw', 'data'),
					  txt_list=os.path.join(data_root, 'raw', 'list_cvt', 'AID11_split2_train.txt'),
					  sampler=train_sampler,
					  force_color=True,
					  video_transform=transforms.Compose([
										 transforms.RandomScale(make_square=True,
	...
	val_sampler   = sampler.SequentialSampling(num=clip_length,
											   ...
											   shuffle=True)
	val   = VideoIter(video_prefix=os.path.join(data_root, 'raw', 'data'),
					  txt_list=os.path.join(data_root, 'raw', 'list_cvt', 'AID11_split2_test.txt'),
					  sampler=val_sampler,
					  force_color=True,
					  video_transform=transforms.Compose([
...
def creat(name, batch_size, num_workers=16, **kwargs):
	...
	if name.upper() == 'UCF101_DARK':
		train, val = get_ucf101_dark(**kwargs)
