# -*- coding: utf-8 -*-
"""
Created on Sat Jan 18 16:13:36 2020
...
class MFNET_BASE_CHANGE(nn.Module):

	def __init__(self, num_classes, pretrained=False, **kwargs):
		super(MFNET_BASE_CHANGE, self).__init__()
...
#       self.softmax = nn.Softmax()
        ...
        self.conv6 = nn.Sequential(OrderedDict([
					('conv', nn.Conv3d( 3, conv5_num_out, kernel_size=(8,7,7), padding=(7,6,6), stride=(1,1,1), bias=False)),
					('bn', nn.BatchNorm3d(conv1_num_out)),
					('relu', nn.ReLU(inplace=True))
					]))
		self.maxpool2 = nn.MaxPool3d(kernel_size=(3,2,2), stride=(1,1,1), padding=(2,1,1))  #???

        self.conv7 = nn.Sequential(OrderedDict([
					("B%02d"%i, MF_UNIT(num_in=conv6_num_out if i==1 else conv7_num_out,
										num_mid=num_mid,
										num_out=conv7_num_out,
										stride=(3,1,1) if i==1 else (0,1,1),
										g=groups,
										first_block=(i==1))) 
                     ]))
...
if __name__ == "__main__":
	...
	# ---------
	net = MFNET_BASE_CHANGE(num_classes=100, pretrained=False)
