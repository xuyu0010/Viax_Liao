...
class Motion_Exctractor_MEAN(nn.Module):
	...
	def forward(self, h):
		...
		h_maxk = max((1,3))
		h_top3_max, pred = h_flatten.topk(h_maxk, 3, True, True)
#		print(h_maxk)
#		print(h_top5_max.shape)
		h_top3_max = self.mh_up2(h_top3_max)
		...
		indices_h_cen = torch.matmul(h_top3_max, indices_h_cen.view(-1,2))
...
class MFNET_THREEP_LINEAR_FRAME(nn.Module):

	def __init__(self, num_classes, pretrained=False, **kwargs):
		super(MFNET_THREEP_LINEAR_FRAME, self).__init__()
...
if __name__ == "__main__":
	...
	# ---------
	net = MFNET_THREEP_LINEAR_FRAME(num_classes=100, pretrained=False)
