...
########attention########
		...
		u = torch.tanh(torch.matmul( weight_W, indices_h_cen))
...
class MFNET_FIVEP_LINEAR_FRAME_CONV3(nn.Module):

	def __init__(self, num_classes, pretrained=False, **kwargs):
		super(MFNET_FIVEP_LINEAR_FRAME_CONV3, self).__init__()
		...
		self.motion_exctractor = Motion_Exctractor_MEAN( inplanes=conv3_num_out, 
														iterplanes=conv2_num_out, 
														outplanes=conv1_num_out*3, num_embedding=20)
		...
		self.classifier = nn.Linear(conv5_num_out + conv1_num_out*21, num_classes)
...
if __name__ == "__main__":
	...
	# ---------
	net = MFNET_FIVEP_LINEAR_FRAME_CONV3(num_classes=100, pretrained=False)
