...
class Motion_Exctractor_Max(nn.Module):
	def __init__(self, inplanes, iterplanes, outplanes, num_embedding=20):
		...
		super(Motion_Exctractor, self).__init__()
...
class Motion_Exctractor_Max_Min(nn.Module):
	def __init__(self, inplanes, num_embedding=20):

		super(Motion_Exctractor_Max_Min, self).__init__()
	...
	def forward(self, h):
		...
		indices_h_max = torch.argmax(h_flatten, dim=3).view(dim_total, -1)
		indices_h_min = torch.argmin(h_flatten, dim=3).view(dim_total, -1)
		indices_h_max = torch.cat(((indices_h_max / W).view(-1,1), (indices_h_max % W).view(-1,1)), dim=1).view(N, C, F, 1, -1)
		indices_h_min = torch.cat(((indices_h_min / W).view(-1,1), (indices_h_min % W).view(-1,1)), dim=1).view(N, C, F, 1, -1)
...
class MFNET_SP_LINEAR_DUAL(nn.Module):
	...
	def __init__(self, num_classes, pretrained=False, **kwargs):
		...
		self.motion_exctractor = Motion_Exctractor_Max_Min( inplanes=conv4_num_out, num_embedding=14)
