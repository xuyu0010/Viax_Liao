...
class MFNET_FIVEP_LINEAR_FRAME_CONV5(nn.Module):

	def __init__(self, num_classes, pretrained=False, **kwargs):
		super(MFNET_FIVEP_LINEAR_FRAME_CONV5, self).__init__()
		...
		self.motion_exctractor = Motion_Exctractor_MEAN( inplanes=conv5_num_out, 
														iterplanes=conv4_num_out, 
														outplanes=conv3_num_out, num_embedding=20)
		...
		self.classifier = nn.Linear(conv5_num_out + conv3_num_out*7, num_classes)
...
if __name__ == "__main__":
	...
	# ---------
	net = MFNET_FIVEP_LINEAR_FRAME_CONV5(num_classes=100, pretrained=False)
